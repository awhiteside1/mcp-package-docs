...
async function testGoPackageDoc(packageName) {
	...
	try {
		...
		try {
			...
			if (response.data) {
				console.log(
					"Successfully retrieved documentation from pkg.go.dev API:",
				);
				console.log("---");
				console.log("Synopsis:", response.data.Synopsis || "None");
				console.log(
					"Documentation available:",
					response.data.Documentation ? "Yes" : "No",
				);
				console.log("---");
				return;
			}
		...
		try {
			console.log(
				"Attempting to fetch documentation from pkg.go.dev website...",
			);
			...
			console.log(
				`Error fetching from pkg.go.dev website: ${webError.message}`,
			);
		}
