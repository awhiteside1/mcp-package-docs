...
export const isSearchDocArgs = (args: unknown): args is SearchDocArgs => {
	return (
		...
		["go", "python", "npm", "swift", "rust"].includes(
			(args as SearchDocArgs).language,
		) &&
...
export class SearchUtils {
		...
		switch (language) {
			...
			case "swift": {
				const swiftMatch = firstLine.match(
					/^(class|struct|enum|protocol|func|var|let)\s+(\w+)/,
				);
				return swiftMatch?.[2];
			}
			case "rust": {
				const rustMatch = firstLine.match(
					/^(pub\s+)?(struct|enum|trait|impl|fn|mod|type)\s+(\w+)/,
				);
				return rustMatch?.[3];
			}
			default:
	 ...
	 */
	public parseNpmDoc(data: { description?: string; readme?: string }): Array<{
		content: string;
		type: string;
	}> {
		const sections: Array<{ content: string; type: string }> = [];
			...
			for (const section of readmeSections) {
				...
				if (content) {
					...
					if (lowerHeading.includes("install")) type = "installation";
						...
						lowerHeading.includes("function")
					)
						type = "api";
	...
	public extractDocSections(readme: string): {
		...
		for (const section of sections) {
			...
			} else if (heading.includes("example") || heading.includes("demo")) {
		...
		if (!result.api) {
			...
			const apiCodeBlocks = readme.match(
				/```(?:js|javascript)[\s\S]*?axios\.(?:get|post|put|delete|patch|request)[\s\S]*?```/g,
			);
			...
			if (
				...
				readme.includes("axios(config)")
			) {
				const configSection = readme.match(
					/(?:Request|Config) (?:Config|Options)[\s\S]*?```[\s\S]*?```/i,
				);
				if (configSection) {
					result.api += `\n\n## Request Config\n\n${configSection[0]}`;
				}
	...
	public extractRelevantContent(readme: string): string {
		...
		if (sections.length > 0 && !sections[0].startsWith("#")) {
			...
			const introContent = introParagraphs
				.slice(0, Math.min(3, introParagraphs.length))
				.join("\n\n");
		...
		for (let i = 0; i < sections.length; i++) {
			...
			for (const keyword of usefulKeywords) {
				if (heading.includes(keyword)) {
					shouldInclude = true;
					this.logger.debug(
						`Including section due to keyword match: ${heading} (matched: ${keyword})`,
					);
					break;
				}
			}
			...
			if (relevantSections.length <= 1) {
				for (let i = 0; i < Math.min(3, sections.length); i++) {
					if (
						...
						!relevantSections.includes(sections[i])
					) {
		...
		if (relevantSections.length === 0) {
			this.logger.debug(
				"No relevant sections found, returning truncated README",
			);
