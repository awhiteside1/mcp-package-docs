import type TypeScriptLspClient from "./lsp/typescript-lsp-client.js";
...
export function getToolDefinitions(
	lspEnabled: boolean,
	lspClient: TypeScriptLspClient | undefined,
) {
			...
			inputSchema: {
				...
				properties: {
					package: {
						type: "string",
						description:
							"Package URL (e.g. https://github.com/apple/swift-argument-parser)",
					},
					symbol: {
						type: "string",
						description:
					...
					projectPath: {
						type: "string",
						description:
							"Optional path to project directory for Package.swift file",
					},
				},
				required: ["package"],
			},
			...
			inputSchema: {
				...
				properties: {
					...
					maxLength: {
						type: "number",
						description:
							"Optional maximum length of the returned documentation",
					},
					query: {
						type: "string",
						description:
							"Optional search query to filter documentation content",
					},
				},
				required: ["package"],
			},
				...
				inputSchema: {
					...
					properties: {
						...
						character: {
							type: "number",
							description:
								"Zero-based character offset for completion position",
						},
						projectRoot: {
							type: "string",
							description:
