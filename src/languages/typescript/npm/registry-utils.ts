...
export class RegistryUtils {
	 ...
	 */
	public getRegistryConfigForPackage(
		packageName: string,
		projectPath?: string,
	): NpmConfig {
		...
		if (packageName.startsWith("@")) {
			...
			return (
				this.registryMap.get(scope) ||
				this.registryMap.get("default") || {
	...
	private loadNpmConfig(projectPath?: string): Map<string, NpmConfig> {
			...
			for (const dir of paths.reverse()) {
				...
				if (existsSync(localNpmrcPath)) {
					...
					try {
						...
						this.logger.error(
							`Error reading local .npmrc at ${localNpmrcPath}:`,
							error,
						);
					}
				}
		...
		try {
			...
			for (const [scope, registry] of scopeToRegistry.entries()) {
				...
				this.logger.debug(`Setting config for scope ${scope}:`, {
					registry,
			...
			this.logger.debug(
				"Final registry configurations:",
				Object.fromEntries(
					Array.from(registryMap.entries()).map(([k, v]) => [
						k,
