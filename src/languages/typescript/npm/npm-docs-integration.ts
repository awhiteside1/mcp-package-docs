...
export class NpmDocsHandler {
	...
	public async describeNpmPackage(
		...
		logger.debug(
			`Getting NPM documentation for ${packageName}${version ? `@${version}` : ""}`,
		);

		try {
			// Check if package is installed locally first
			const isInstalled = isNpmPackageInstalledLocally(
				packageName,
				projectPath,
			);
					...
					if (existsSync(packageJsonPath)) {
						...
						if (packageInfo.types || packageInfo.typings) {
							...
							if (existsSync(typesPath)) {
								...
								apiDocumentation = await this.enhancer.extractApiDocumentation(
									packageName,
									typesContent,
								);
								...
								if (apiDocumentation && apiDocumentation.exports.length > 0) {
									const apiMarkdown =
										this.enhancer.formatApiDocumentationAsMarkdown(
											apiDocumentation,
										);
									localDoc.usage = localDoc.usage
										? `${localDoc.usage}\n\n${apiMarkdown}`
										: apiMarkdown;
								}
							}
			...
			logger.debug(
				`Fetching NPM documentation for ${packageName} from registry`,
			);
			...
			try {
				...
				if (packageInfo) {
					...
					if (packageInfo.readme) {
						...
						const readme = this.enhancer.convertHtmlToMarkdown(
							packageInfo.readme,
						);
					...
					if (includeTypes) {
						const typesContent = await this.enhancer.fetchTypeDefinition(
							packageName,
							version,
						);
			...
			logger.error(
				`Error getting NPM documentation for ${packageName}:`,
				error,
			);
			return {
	...
	public async getNpmPackageDoc(
		...
		logger.debug(
			`Getting full NPM documentation for ${packageName}${version ? `@${version}` : ""}`,
		);
			...
			try {
				...
				if (includeTypes) {
					...
					if (typesContent) {
						...
						if (
							apiDocumentation &&
							(apiDocumentation.exports.length > 0 ||
								apiDocumentation.types.length > 0)
						) {
				...
				const errorMessage =
					error instanceof Error ? error.message : String(error);
				logger.error(
					`Error getting full NPM documentation for ${packageName}:`,
					error,
				);
				return {
					error: `Failed to fetch NPM documentation: ${errorMessage}`,
