...
export class RustDocsHandler {
	...
	async getCrateDetails(crateName: string): Promise<{
		...
		try {
			...
			const response = await rustHttpClient.cratesIoFetch(
				`crates/${crateName}`,
			);
			...
			this.logger.error(`error getting crate details for: ${crateName}`, {
				error,
			});
			throw new Error(
	...
	async getSourceCode(
		...
		try {
			this.logger.info(
				`Getting source code for ${path} in crate: ${crateName}`,
			);
