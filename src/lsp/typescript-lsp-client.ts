...
export class TypeScriptLspClient {
	...
	private diagnosticsListeners: Map<
		string,
	...
	public async getOrCreateServer(
		...
		const languageServer = this.languageServers.get(serverKey);
		if (languageServer) {
		...
		connection.onNotification((method, params) => {
			this.logger.debug(
				`[${serverKey}] Notification received:`,
				method,
				params,
			);
		});
		...
		const actualRoot =
			projectRoot && existsSync(projectRoot) ? projectRoot : process.cwd();
		...
		try {
			...
			this.logger.debug(
				`Initialize result for ${serverKey}:`,
				initializeResult,
			);
	...
	public async getHover(
		...
		try {
			...
			const hover: Hover = await server.connection.sendRequest(
				"textDocument/hover",
				{
	...
	private async getLanguageServerConfig(
		languageId: string,
		...
		if (!commandAvailable) {
			this.logger.debug(
				`Command ${commandName} not found, attempting to install...`,
			);
