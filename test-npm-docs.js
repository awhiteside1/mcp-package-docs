...
async function testNpmDocs() {
	try {
		...
		console.log(
			`Fetched TypeScript definitions: ${typesContent ? "Yes" : "No"}`,
		);

		if (typesContent) {
			// Test extracting API documentation
			console.log("\nTesting API documentation extraction...");
			const apiDoc = await enhancer.extractApiDocumentation(
				"axios",
				typesContent,
			);
			console.log(
				`Extracted API documentation: ${apiDoc.exports.length} exports, ${apiDoc.types.length} types`,
			);
		...
		// Mock functions
		const getRegistryConfigForPackage = () => ({
